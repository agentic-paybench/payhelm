...
First-party measurement where adapters already run (R1 Base, R9 Solana, R3 Lightning regtest → R11 proxy); rail docs + public telemetry for the rest and for spread/tail. Documentary track done (table above). First-party track: **R1 + R9 + R2 + R10 done (n=30 each, 0 failures, 2026-06-04)** — empirical distributions now replace the doc-derived medians (harnesses: `poc/rail-x402-base/measure_finality.py`, `poc/rail-solana-x402/src/measure-finality.ts`, `poc/rail-stellar-x402/src/measure-finality-manual.ts`, `poc/rail-tempo-mpp/src/measure-finality.ts`). R2 = manual path (spec x402-on-Stellar validated end-to-end with the OZ key; spec re-measure deferred). **R10 = real-rail MPP-on-Tempo** (mppx, Moderato testnet) — supersedes its doc-proxy/LOW + "mock per Variant E" status. **Remaining:** R11 (Lightspark/Spark) — verified buildable now (self-serve Spark regtest, same MPP/mppx family as R10); build pending.
