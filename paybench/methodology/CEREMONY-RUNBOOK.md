...
**Do not start until the cross-LLM adversarial review (step 2) clears with no decisive refutation of
the frozen method.** Anchoring commits the manifest hash permanently; if the review reopens D1/D2,
re-freeze first, then regenerate the manifest, then run this.

**The anchored value** = the manifest hash
...
your laptop with the repo checked out at commit `58f656e1`.
...
python3 -m paybench.mockbench.cli run | grep run_hash    # == 895f99ed…14ee0
```
...
git tag -s paybench-prereg-v1.0 58f656e1 \
  -m "PayBench settlement-finality pre-registration — methodology v1.0; manifest 6e65aff9…967a76"
git tag -v paybench-prereg-v1.0                # verify the signature
git push origin paybench-prereg-v1.0           # requires the commit to be pushed (step 6)
```
...
| Signed git tag | pending | `paybench-prereg-v1.0` |
| OSF DOI | pending | DOI: — |
