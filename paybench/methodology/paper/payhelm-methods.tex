...
\date{Methodology v1.2 (frozen 2026-06-06). Preprint v1.}
...
primitives. PayBench's original contribution is the per-rail-canonical reliance-level finality doctrine
(\S\ref{sec:finality}) and the calibrated mock-fixture methodology (Bradley--Terry MLE with a symmetric
smoothing prior, $\mathrm{pass}@k = P(\text{finality} \le k)$, and Wilson lower-bound intervals), for
which this pre-registration establishes precedence. We do not frame the canonicalisation or
...
tag \texttt{paybench-prereg-v1.2}. The manifest hash committed by the anchors is
\texttt{sha256:a5f6feb4\ldots d3a6f} (full value in the repository manifest).
