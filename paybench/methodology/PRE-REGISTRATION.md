...
- **Methodology version:** v1.0 (frozen 2026-06-05)
- **Decisions confirmed:** 2026-06-06 — D1-keep (per-rail-canonical reliance-level finality
  doctrine), D2-freeze (empirical `sigma_log` as-measured + disclosed limitation)
- **Frozen-method commit (agentic-paybench/payhelm):** `58f656e1`
- **Manifest:** `paybench/methodology/prereg-manifest.sha256`
- **Manifest hash (the value the cryptographic anchors commit to):**
  `sha256:6e65aff9943660393784847827c21be07697717d96e957eeb79afab2ca967a76`
...
flatter a rail. The full method is `paybench/methodology/methodology.md` (v1.0); this document is the
...
|---|---|
| Settling rail set (race on finality) | R1 x402-Base, R2 x402-Stellar, R9 x402-Solana, R10 MPP-on-Tempo, R11 MPP-on-Lightning |
...
| Per-rail finality definitions | §3 — ecosystem-canonical reliance level per rail (Base soft@1-block; Stellar ledger-close; Solana `finalized`; Tempo BFT block; Lightning preimage via Spark) |
...
- Scored mock run is byte-deterministic; **run hash**
...
- **Signed git tag** on commit `58f656e1` (YubiKey OpenPGP)
...
sha256sum methodology/prereg-manifest.sha256           # == 6e65aff9…967a76
python3 -m paybench.mockbench.cli verify               # fixture hashes vs provenance
python3 -m paybench.mockbench.cli run                  # reproduces run_hash 895f99ed…14ee0
```
