...
| **R9 x402-Solana** | `rail-solana-x402` (TS, Express, **hosted** facilitator) | ✅ **run-validated** (2026-06-23) A=400 ms / B=2.2 ms, 30/30 ok | `npx ts-node src/server.ts` + `src/measure-rapl.ts` (devnet `BUYER_PRIVATE_KEY`; buyer funded). See pilot log R9 |
| **R2 x402-Stellar** | `rail-stellar-x402` (TS, Express, OZ hosted facilitator) | ✅ **run-validated** (2026-06-23) A=464 ms / B=2.2 ms, 30/30 ok | `npx ts-node src/server.ts` + `src/measure-rapl.ts` (`STELLAR_BUYER_SECRET` + `OZ_X402_TESTNET_KEY`; buyer USDC 19.97). See pilot log R2 |
| **R10 MPP-Tempo** | `rail-tempo-mpp` (TS, `route()` handler) | ✅ **A+B run-validated** (B=2.95 ms 2026-06-23; **A=19.5 ms** session voucher 2026-06-24) | B: `src/measure-rapl.ts` (unpaid `GET /data`→402, no signer). A: `src/measure-rapl-session.ts` (mppx **session** intent — channel open once + voucher accept; funded buyer). Charge excluded (verify+settle fused). **A is local-crypto + periodic ~5 s TTL RPC refresh** (novel 3rd A class). ESM/nodenext |
| **R11 MPP-Spark-Lightning** | `rail-lightning-mpp` (TS, `route()`) | ✅ **B run-validated** (2026-06-23) B=1252 ms warm (invoice-mint), 15/15 ok | `npx tsx src/server.ts` + `src/measure-rapl.ts` — **B only** (unpaid `GET /data`→402 = BOLT11 invoice + macaroon issuance; no buyer wallet). **B-only by methodology** (no A). Intrinsic Spark invoice-generation hop (≫ x402's local 402 — the heterogeneous-B outlier). ESM/nodenext. (NB `rail-lightning-l402/` is a separate older Python "Rail 3" L402, not this dim-2 rail) |
| **R6 GCP+AP2** | `poc/rail-ap2` (Python, capture-replay) | ✅ **done + run-validated, blocker-fixed (2026-06-24)** — **headline: DPC 1.58 ms** (2 ES256); variant: human-present 0.68 ms (1 ES256) | `measure_rapl_ap2_full.py` replays a captured mandate n× LLM-free, crypto-only window (keys pre-loaded, logging no-op'd). **Two variants** (founder: DPC = headline): **delegated/DPC** (`~~` chain, human-not-present flow, 1.58 ms) + **human-present** (issuer-only single token, 0.68 ms). **A only** (in-process; no B, no RTT floor). Old 1.68 ms was disk-I/O-contaminated (superseded). See pilot log R6 + `CAPTURE-FULL.md` |
